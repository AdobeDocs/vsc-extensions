  ...
  "function": function AM009(params, onError) {
        ...
        if (i > 1) {
            ...
            if (!incodeblock) {
                if (
                    ...
                    if (warn) {
                        shared.addWarningContext(params.name, i + params.frontMatterLines.length, prevline, module.exports.names[0] + '/' + module.exports.names[1] + ' AFM (NOTE) block line missing ">"')
                    } else {
                        shared.addErrorDetailIf(onError, i, null, 'No > on AFM (NOTE) block line: ' + prevline.trim());
                    }
                }
            }
        }
        ...
        if (!token.line.startsWith("<!--")) {
            lines.forEach(function forLine(line, lineNumber) {
                if (containsAfmTag(line)) {
                    shared.addErrorDetailIf(onError, lineNumber + token.lineNumber, null, "Admonitions not supported in HTML: " + line.trim());
                }
            })
        }
    })
    shared.forEachLine(function forLine(line, lineIndex) {
        ...
        if (!incodeblock) {
            ...
            if (line.match(/\[!UICONTROL\s[^\]]*?\]\(/) || line.match(/\[!DNL\s[^\]]*?\]\(/)) {
                shared.addErrorContext(onError, lineIndex + 1, line);
            }
    ...
    shared.filterTokens(params, "blockquote_open", function forToken(token) {
        ...
        for (var i = oline; i < cline; i++) {
            ...
            if (lineindent != indent){
                if (lineindent < 0) {
                    shared.addErrorDetailIf(onError, i + 1, indent.toString(), lineindent, 'Missing blockquote marker (>) or newline')
                } else {
                    shared.addErrorDetailIf(onError, i + 1, indent.toString(), lineindent, 'Mismatched Indent for Block Quote')
                }
            }
        }
        // console.log(token)
